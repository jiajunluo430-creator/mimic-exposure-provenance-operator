# JBI first-submission upload map
...
Figure 1 through Figure 5 are the data-first JBI redesign finalized on 2026-08-05. The scientific values and figure-facing source tables are unchanged; only visual grammar and layout were redesigned. The upload PDFs are editable vector artwork, and the TIFF files are raster alternatives. The manuscript contains captions and callouts but no embedded figure media, so no duplicate old artwork remains inside the DOCX.
